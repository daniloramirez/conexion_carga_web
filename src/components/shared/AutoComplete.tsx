...
function AutoComplete<T>(props: AutoCompleteProps<T>) {
    ...
    useEffect(() => {
        ...
        setOptions(items)
    }, [data, maxResults, optionKey, value])
    ...
    useEffect(() => {
        const referencia = refs.domReference.current
        const inputEnFoco =
            typeof document !== 'undefined' &&
            referencia &&
            document.activeElement === referencia

        if (inputEnFoco && options.length > 0) {
            setOpen(true)
    ...
    }, [open, options, refs.domReference])
